...
def sweep_values():
    global gateway_distance_max, gateway_distance_min, gateway_distance_max_q, gateway_distance_min_q, distance_accountable_max, z_max, num_states
    ...
    mid_pos = (gateway_distance_min + gateway_distance_max) / 2
    positions = list(zip(np.linspace(gateway_distance_min, mid_pos, steps, endpoint=False),
                         np.linspace(gateway_distance_max, mid_pos, steps, endpoint=False)))

    with open("out_{}.txt".format(policy_name), "a+") as file:
        for i, pos in enumerate(positions):
            # gateway_distance_max -= dgw
            # gateway_distance_min += dgw
            gateway_distance_min, gateway_distance_max = pos
            ...
            infinite = True
            if infinite:
                path_file = "{}-{}-infinite.p".format(gateway_distance_max_q, gateway_distance_min_q)
            else:
                path_file = "{}-{}.p".format(gateway_distance_max_q, gateway_distance_min_q)
            ...
            """
            policy_unravelled = np.array(policy).reshape((x_max, y_max, z_max))
            policy_2 = pickle.load(open('/home/ruben/mdp-ram-infinite.p', 'rb'))
            policy_unravelled_2 = np.array(policy_2).reshape((x_max, y_max, z_max))
            ...
            for num_nodos in [12]:  # range(2, 70):
                clf = tree.DecisionTreeClassifier(random_state=0, max_leaf_nodes=num_nodos, criterion="gini",
                                                  class_weight={0: 1, 1: 1, 2: 5})
                clf.fit(X, Y)
                rights = sum(clf.predict(X) == Y)
                print("Precission con {} nodos = {}%".format(num_nodos, rights / len(policy) * 100))
            ...
            print('Reward {} para pos = {} -> mean: {}, std: {}'.format(policy_name, pos, mean_r, std_r))
            file.write('Reward {} para pos = {} -> mean: {}, std: {}\n'.format(policy_name, pos, mean_r, std_r))
            file.flush()
...
def re_compute_policy(path_file, infinite=False):
    ...
    print('Computing policy for lambda = {}. Distance between gateways {} - {}'.format(gen_prob, gateway_distance_min,
                                                                                       gateway_distance_max))
    ...
    if infinite:
        pi = mdptoolbox.mdp.RelativeValueIteration(T, R, epsilon=0.005, max_iter=3000)
        pi.run()
