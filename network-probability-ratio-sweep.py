...
def sweep_values():
    ...
    probs_ratio = np.round(np.linspace(1, 5, 11), 3)
    ...
    with open("out_{}.txt".format(policy_name), "a+") as file:
        for i, ratio in enumerate(probs_ratio):
            ...
            gen_prob[0] = 0.9
            gen_prob[2] = 0.1 / (1 + ratio)
            gen_prob[1] = ratio * gen_prob[2]
            ...
            infinite = True
            if infinite:
                path_file = "{}-infinite.p".format(ratio)
            else:
                path_file = "{}.p".format(ratio)
            ...
            print('Reward {} para pos = {} -> mean: {}, std: {}'.format(policy_name, ratio, mean_r, std_r))
            file.write('Reward {} para pos = {} -> mean: {}, std: {}\n'.format(policy_name, ratio, mean_r, std_r))
            file.flush()
